...
frappe.ui.form.on("Room Booking", {
	...
	delivery_date: function(frm) {
		$.each(frm.doc.items || [], function(i, d) {
			if(!d.checkin_date) d.checkin_date = frm.doc.checkin_date;
		});
		refresh_field("items");
	},
...
frappe.ui.form.on("Booking Item", {
	item_code: function(frm,cdt,cdn) {
		...
		if (frm.doc.checkin_date) {
			row.checkin_date = frm.doc.checkin_date;
			refresh_field("checkin_date", cdn, "items");
		} else {
			frm.script_manager.copy_from_first_row("items", row, ["checkin_date"]);
		}
	},
	delivery_date: function(frm, cdt, cdn) {
		if(!frm.doc.checkin_date) {
			erpnext.utils.copy_value_in_all_rows(frm.doc, cdt, cdn, "items", "checkin_date");
		}
	}
});
