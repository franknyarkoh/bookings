...
class SalesOrder(SellingController):
	...
	def validate_po(self):
		# validate p.o date v/s checkin date
		if self.po_date:
			for d in self.get("items"):
				if d.checkout_date and getdate(checkin_date) > getdate(d.checkout_date):
					frappe.throw(_("Row #{0}: Expected Checkout Date cannot be before Checkin Date")
						.format(d.idx))
	...
	def validate_delivery_date(self):
		if self.order_type == 'Sales':
			...
			if self.delivery_date:
				...
				frappe.throw(_("Please enter Checkout Date"))
